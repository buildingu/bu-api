    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            <table class="tables-fixed w-full">
                ...
                <tbody>
                    @if (sizeof($educationalLevel) == 0)
                        <tr>
                            <td class="border px-4 py-2 text-center" colspan="3">No countries found.</td>
                        </tr>
                    @else
                        @foreach($educationalLevel as $educational_level)
                            <tr class="class-center">
                                <td class="border px-4 py-2 text-center">{{ $educational_level->id }}</td>
                                <td class="border px-4 py-2 text-center">{{ $educational_level->name }}</td>        
