            ...
            <table class="tables-fixed w-full">
                ...
                <tbody>
                    @if (sizeof($institutions) == 0)
                        <tr>
                            <td class="border px-4 py-2 text-center" colspan="3">No institutions found.</td>
                        </tr>
                    @else
                        @foreach($institutions as $institution)
                            <tr class="class-center">
                                <td class="border px-4 py-2 text-center">{{ $institution->id }}</td>
                                <td class="border px-4 py-2 text-center">{{ $institution->name }}</td>
                                <td class="border px-4 py-2 text-center">{{ $institution->country_id}}</td>          
